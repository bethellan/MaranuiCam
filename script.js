...
async function fetchData() {
  ...
  try {
    ...
    const data = {
      ...
      offline:false
    /* ---------- Fetch + align Open-Meteo data (debug version) ---------- */
async function fetchData() {
  ...
  try {
    console.log("⏳ Fetching forecast:", forecast);
    ...
    console.log("✅ Forecast response:", fR);
    console.log("✅ Marine response:", mR);
    console.log("✅ Tide response:", tR);

    if (!fR.value?.ok) console.warn("⚠️ Forecast fetch failed", fR.reason || fR.value);
    if (!mR.value?.ok) console.warn("⚠️ Marine fetch failed", mR.reason || mR.value);
    if (!tR.value?.ok) console.warn("⚠️ Tide fetch failed", tR.reason || tR.value);

    const f = fR.value && fR.value.ok ? await fR.value.json() : {};
    const m = mR.value && mR.value.ok ? await mR.value.json() : {};
    const t = tR.value && tR.value.ok ? await tR.value.json() : {};
    console.log("📦 Parsed data:", { f, m, t });
    ...
    console.log("🏄‍♂️ Fetched data assembled:", data);
    return data;
