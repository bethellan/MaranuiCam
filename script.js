...
function toggleCamera() {
  ...
  setTimeout(() => {
    ...
    switchBtn.setAttribute('aria-pressed', showingSurf);
    ...
    setTimeout(() => frame.classList.remove('fade'), 50);
  }, 300);
}
...
// Fallback generator (if live calls fail) - IMPROVED
function makeFallback(){
  ...
  // Set realistic sunrise/sunset for Lyall Bay
  const sunrise = new Date(base);
  sunrise.setHours(6, 42, 0, 0); // 6:42 AM
  const sunset = new Date(base);
  sunset.setHours(19, 58, 0, 0); // 7:58 PM
  
  const nightCols = new Set();
  hours.forEach((h, idx) => {
    if (h < sunrise || h >= sunset) nightCols.add(idx);
  });
  
  return {
    labelHours: hours, 
    wave, waveP, wind, gusts, windDir, rain, tide, 
    nightCols, sunrise, sunset, offline: true
  };
}

// Fetch Open-Meteo with resilience - FIXED error handling
async function fetchData(){
  ...
  try {
    ...
    return {
      labelHours, wind, gusts, windDir, rain, wave, waveP, waveD, tide, 
      nightCols, sunrise: sunrise0, sunset: sunset0, offline: false
    };
  } catch(e) {
...
// Surfability score (per-hour) - IMPROVED
function hourlyScore(wave, wind, rain, dir) {
  let s = 0;
  
  // Wave height scoring (optimal around 1.0-1.3m for Lyall Bay)
  if (wave != null) {
  ...
  // Direction scoring (SW to NW is best for Lyall Bay)
  if (typeof dir === 'number') {
    if (dir >= 285 && dir <= 325) s += 1;
    else if ((dir >= 260 && dir < 285) || (dir > 325 && dir <= 340)) s += 0.5;
  }
  
  return Math.max(0, Math.min(10, +s.toFixed(1)));
}

// Build table - FIXED table rendering
function buildTable(d) {
  ...
  addRow('Period (s)', d.waveP, 
    (v) => v == null ? '—' : v.toFixed(0)
  );

  addRow('Wind (km/h)', d.wind,
  ...
  addRow('Gusts (km/h)', d.gusts,
    (v) => v == null ? '—' : Math.round(v)
  );

  addRow('Direction', d.windDir,
    (v) => {
      if (v == null) return '—';
  ...
  addRow('Rain (mm/hr)', d.rain,
    (v) => v == null ? '—' : v.toFixed(1)
  );

  addRow('Tide (m)', d.tide,
    (v) => v == null ? '—' : v.toFixed(2)
  );

  // Calculate and add surfability scores
  const scores = d.labelHours.map((_, i) => 
    hourlyScore(d.wave[i], d.wind[i], d.rain[i], d.windDir[i])
  );
  
  addRow('Surfability (1–10)', scores,
    (v) => v == null ? '—' : v.toFixed(1),
    (v) => {
      if (v == null) return '';
  ...
  // Update current surfability badge
  ...
  if (nowScore != null) {
    ...
    else if (nowScore >= 5) badge.classList.add('fair');
...
// Tide highs/lows from tide series - FIXED syntax error
function computeNextTides(times, heights) {
  try {
    ...
    console.warn('Tide computation failed:', e);
    return { nextHigh: null, nextLow: null };
  }
}

// Main refresh function - FIXED tide display logic
async function refresh() {
  try {
    const d = await fetchData();
    
    // Update sunrise/sunset labels
    document.getElementById('sunriseLabel').textContent = d.sunrise ? 
      d.sunrise.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—';
    document.getElementById('sunsetLabel').textContent = d.sunset ? 
      d.sunset.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—';

    // Get tide data for high/low tide times
    let tideHigh = '—', tideLow = '—';
    
    if (!d.offline && d.tide.some(t => t !== null)) {
      try {
        // Use the tide data we already have from fetchData
        const { nextHigh, nextLow } = computeNextTides(
          d.labelHours.map(h => h.toISOString()),
          d.tide
        );
        
        tideHigh = nextHigh ? nextHigh.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—';
        tideLow = nextLow ? nextLow.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}) : '—';
      } catch(e) {
        console.warn('Tide time extraction failed:', e);
      }
    }
    
    document.getElementById('tideHigh').textContent = tideHigh;
    document.getElementById('tideLow').textContent = tideLow;
